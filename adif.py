...
<eoh>\n""" % (dt, len(records), len(str(ADIF_VERSION)), ADIF_VERSION))
            ...
            n = 0
            for r in records:
                if "TIME_ON" not in r or not r["TIME_ON"]:
                    continue # Do not report QSO's where time is not given
                if "PROP_MODE" in r:
                    if r["PROP_MODE"] == "A":
                        r["PROP_MODE"] = "AUR"
                    if r["PROP_MODE"] == "T":
                        r["PROP_MODE"] = "TR"
                    if r["PROP_MODE"] not in PROPAGATION_MODES:
                        print("PROP MODE %s is not valid" % r["PROP_MODE"])
                        continue
                    if "BAND" in r and r["BAND"]:
                        for i in range(1, len(BANDS_RANGES)):
                            fq_range = BANDS_RANGES[i]
                            if fq_range and fq_range[0] <= float(r["BAND"]) <= fq_range[1]:
                                r["FREQ"] = r["BAND"]
                                r["BAND"] = BANDS[i]
                                break
                if "MODE" in r:
                    if r["MODE"] == "<None>" or r["MODE"] == None:
                        r["MODE"] = ""
                    if r["MODE"] not in MODES:
                        print("Transmit mode %s is not valid" % r["MODE"])
